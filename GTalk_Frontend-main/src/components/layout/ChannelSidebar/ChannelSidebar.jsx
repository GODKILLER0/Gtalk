...
function ChannelSidebar({ activeChannelId, onSelectChannel, isOpen, activeServerId, guildName, channels, onChannelCreated }) {
  ...
  const handleCreateChannel = async () => {
    if (newChannelName.trim() && activeServerId && user) {
      try {
        console.log("Creating channel:", { name: newChannelName.trim(), type: newChannelType, serverId: activeServerId, userId: user.id });
        const newChannel = await channelApi.createChannel(activeServerId, user.id, newChannelName.trim());
        console.log("Channel created:", newChannel);
        // Add category to the new channel and map backend properties to frontend expectations
        const channelWithCategory = {
  ...
  return (
    <aside className={`channel-sidebar ${isOpen ? "open" : ""}`}>
      <div className="channel-header">
        <h3>{guildName || "Select a Guild"}</h3>
        <button
